...
async function startCamera(){
 ...
 try{
  stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:"environment"}}});$("camera").srcObject=stream;await $("camera").play();$("cameraMessage").textContent="กำลังสแกน… วาง Barcode ให้อยู่ในกรอบ";
  if("BarcodeDetector" in window){const formats=await BarcodeDetector.getSupportedFormats();const wanted=["ean_13","ean_8","upc_a","upc_e","code_128","code_39","itf","qr_code"].filter(x=>formats.includes(x));const detector=new BarcodeDetector({formats:wanted.length?wanted:formats});scanTimer=setInterval(async()=>{try{const codes=await detector.detect($("camera"));if(codes.length){const code=codes[0].rawValue;$("barcodeInput").value=code;findBarcode(code);stopCamera();}}catch(_e){}},450)}
  else $("cameraMessage").textContent="กล้องเปิดแล้ว แต่ Safari เครื่องนี้ไม่รองรับ BarcodeDetector ให้กรอกรหัสด้านล่าง";
 }catch(e){toast("ไม่สามารถเปิดกล้องได้ กรุณาอนุญาตการใช้กล้อง")}
}
function stopCamera(){if(scanTimer){clearInterval(scanTimer);scanTimer=null}if(stream){stream.getTracks().forEach(t=>t.stop());stream=null}$("camera").srcObject=null;$("cameraMessage").textContent="กด “เปิดกล้อง” แล้วหันกล้องไปที่ Barcode"}
